...
async function displayAlbum(albumData) {
  ...
  console.log("DATA", albumData.circle.UserCircle)
  const isCircleMember = albumData.circle.UserCircle.find((obj) => {
    if (obj.user.username === currentLocalUser) {
      return obj;
    }
  })
  const addPhotoToAlbum = `<button id="addPhotos" class="col-span-1 justify-self-end w-6 h-6" albumId="${albumData.id}">
  ...
  pageContent.innerHTML = `
    <div id="albumPhotos">
      ...
      <div class="grid grid-cols-2 justify-between place-items-center mt-12 mb-2 mr-0">
        ...
        ${isCircleMember ? addPhotoToAlbum : ""}
      </div>
